...
def fetch_and_load_fng(**context):
    ...
    if conf.get("exec_date"):
        exec_date = pd.to_datetime(conf["exec_date"]).date()
        print(f"[FNG] Using exec_date from conf: {exec_date}")
    else:
        exec_date = context["data_interval_start"].date()
        print(f"[FNG] Using data_interval_start: {exec_date}")
